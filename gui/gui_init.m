...
% Last Modified by GUIDE v2.5 12-Dec-2017 10:44:10
...
trial_data = mean(prewhite(MEGid,:),2);

% setup with fieldtrip (CTF layout)
try handles.thelay;
catch
    cfg.layout = 'CTF275.lay';
    thelay     = ft_prepare_layout(cfg);
    handles.thelay = thelay;
end

x   = handles.thelay.pos(1:length(MEGid),1);
y   = handles.thelay.pos(1:length(MEGid),2);
tri = delaunay(x,y);
trisurf(tri,x,y,trial_data);
